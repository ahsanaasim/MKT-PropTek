...
## Objective
Implement a landing page that follows the locked Gallery Marketplace design system, featuring prominent CTAs for each user type. Ensure login/register flows redirect users to their role-specific home, and restrict access to only the screens/features allowed for their assigned role, per the locked Design Brain prompt.

## Context
## Evidence graph summary
Levels: E0=0 E1=64 E2=0 E3=0
Validation: unverified=64
Note: extraction confidence is not stakeholder approval; treat E1 stakeholder beliefs as hypotheses.

## decision:approved
- id=83554d10-8ae2-4182-9e92-a5d9eef6d55d title="Design patch: Gallery Marketplace" confidence=n/a evidence=e1 validation=unverified description="Our application should include a landing page. The landing page should follow the overall design and branding of the application and provide login options for each user type.

After signing in, users "
...
Screens: Landing Page, Login, Forgot Password, Register, Job Poster Home, Clean Up Team Home, Job Post Wizard: Details, Job Post Wizard: Budget & Deadline, Job Post Wizard: Media Upload, Job Post Wizard: Review & Submit, Job List, Job Detail, Bid List Modal, Bid Approve Modal, Bid Reject Modal, Job Timeline Drawer, Job Chat Screen, Completion Upload Drawer, Completion Approval Modal, Dispute Modal, Feedback & Rating Modal, Payment Modal, Available Jobs List, Available Job Detail, Submit Bid Modal, Edit Bid Modal, Withdraw Bid Modal, Job History (Team), Job History Detail (Team), Job History (Poster), Job History Detail (Poster), Reports & Analytics, Profile & Account, Membership Options Modal, Certification Upload Modal, Notifications Drawer, Search & Filter Drawer, Permission Denied, 404 Not Found
Design system: Vibrant merchandising / gallery storefront with mobile-first, commerce-polished visual hierarchy
Components: BrandLogo, ValuePropText, PrimaryCTAButton, SecondaryCTAButton, StepIllustration, StepText, AnimatedDemo, TestimonialCard, UserAvatar, StarRating, FAQAccordion, LegalLink
...
Colors: primary: #FF385C; secondary: #E31C5F; background: #FFFFFF; neutralBase: warm paper / soft neutrals with premium merchandising contrast
Typography: Headings: Cinzel, serif; Body: Josefin Sans, sans-serif; Font weights: 400 (regular), 600 (semibold), 700 (bold); Sizes: Responsive, mobile-first scale (e.g., 32px/24px/20px/16px/14px/12px); Line heights: 1.2 (headings), 1.5 (body)
Spacing: Base unit: 4px; Mobile padding: 16px sides, 24px top/bottom for main regions; Grid gap: 12px (gallery grid), 16px (form fields), 24px (section spacing); Card padding: 16px–24px; Button min height: 48px
...
- `.ops4/design-system/screens/job-post-wizard-details.md`
- `.ops4/design-system/screens/job-post-wizard-budget-deadline.md`
- `.ops4/design-system/screens/job-post-wizard-media-upload.md`
- `.ops4/design-system/screens/job-post-wizard-review-submit.md`
...
- `.ops4/design-system/screens/available-jobs-list.md`
- `.ops4/design-system/screens/available-job-detail.md`
- `.ops4/design-system/screens/submit-bid-modal.md`
- `.ops4/design-system/screens/edit-bid-modal.md`
- `.ops4/design-system/screens/withdraw-bid-modal.md`
- `.ops4/design-system/screens/job-history-team.md`
- `.ops4/design-system/screens/job-history-detail-team.md`
- `.ops4/design-system/screens/job-history-poster.md`
- `.ops4/design-system/screens/job-history-detail-poster.md`
...
- `.ops4/design-system/screens/permission-denied.md`
...
# Screen blueprint: Job Post Wizard: Media Upload

Route: `/poster/job/new/media`
Purpose: Step 3: Upload photos and PDFs for the job.
Layout: **two-column-main-rail**

## Required regions
- **main**: Step 3: Upload photos and PDFs for the job. — components: PageHeader, PrimaryContent
- **context-rail**: Related context, history, or secondary actions — components: RelatedInfo, ActivityOrMeta
...
# Screen blueprint: Job History (Team)

Route: `/team/history`
Purpose: View all jobs completed or bid on by the team.
Layout: **dashboard-grid**

## Required regions
- **header**: Page title, quick stats, and navigation — components: PageTitle, JobCountMetric, ExportHistoryAction
- **filters**: Refine job history results — components: StatusFilter, DateRangeFilter, PosterFilter, PaymentStatusFilter, SearchBar
...
Index URL: https://github.com/ahsanaasim/MKT-PropTek/blob/main/.ops4/design-system/DESIGN_SYSTEM.md
...
Source: custom (design-intelligence-lock)
...
## Requirements
- Add a Landing Page ('/') with visually prominent PrimaryCTAButton ('Post a Job') and SecondaryCTAButton ('Find Cleanup Jobs'), styled and laid out per the locked design system.
- Both CTAs route to the Login ('/auth/login') screen.
- On Login and Register, after successful authentication, redirect users to their role-specific home: '/poster/home' for Job Posters, '/team/home' for Clean Up Teams.
- Implement access control so users can only view/navigate to screens/features for their assigned role; show Permission Denied ('/denied') if they attempt to access others.
- Do not alter or redesign existing layouts, navigation, or visual system; use only existing component patterns from the Design Memory.
...
- UI MUST be built with shadcn/ui + lucide-react — theme tokens to the locked brand. Do not invent a parallel component library or raw Bootstrap/MUI look.
- NON-INTERACTIVE CLI ONLY: never leave create-next-app or shadcn waiting on arrow-key menus. Use `npx create-next-app@latest . --typescript --tailwind --eslint --app --src-dir --import-alias "@/*" --use-npm --yes` and `npx shadcn@latest init --defaults --force` (or `--preset nova -y --force`). NEVER run `npx shadcn@latest init -y` alone — it still prompts for a preset and hangs Cursor Cloud agents. If a CLI blocks on "Which preset…", kill it and re-run with --defaults/--preset.
...
## Acceptance Criteria
- Landing Page displays both CTAs as described and routes correctly.
- Login/Register flows redirect to the correct home screen based on user role.
- Role-based access control is enforced; unauthorized navigation shows Permission Denied screen.
- No changes to existing layouts, navigation, or design system.
- Lint passes
- Build passes
...
- UI matches locked designMemory brand tokens (colors/typography); Pattern Brain supplies composition geometry only.

## Agent Instructions
Create a new branch named:
`ai-build/issue-3-add-landing-page-with-role-based-login-and-acces`
...
PR title should be:
AI Build: Add landing page with role-based login and access control
